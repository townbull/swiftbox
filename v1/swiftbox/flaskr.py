...
class Global:
  ...
  @staticmethod
  def setsession(conn, group, user):
...
def upload(container):
  ...
  if Global.isloggedin():
    if request.method == "POST":
      ...
      if not filename:
        flash("Please specity a file !!!!", "error")
        msg=Global.swiftconn.list_objects(container)
        return render_template("list_obj.html", \
                container=container, \
                username="%s:%s"%(Global.group, Global.user),\
                 result=msg)
      segsize=request.form["segsize"].strip()
      app.logger.warning("segment size: [%s]"%segsize)
      if segsize and not re.match("^[0-9]+(B|K|M)$", segsize):
        flash("segsize format example: 1B, 1K, 1M.... B:byte, K: kilobytes, M:mega-bytes", "error")
        msg=Global.swiftconn.list_objects(container)
        return render_template("list_obj.html", \
                username="%s:%s"%(Global.group, Global.user),\
               container=container, result=msg)
      if segsize:
        unit=segsize[-1]
        size=int(segsize[:-1])
        if "B" == unit:
      ...
      ct=Global.swiftconn._get_object_info(container, filename)
      if ct[0] == 0:
        flash("objct    %s/%s   already exists. Abort!"%(container, filename))
        msg=Global.swiftconn.list_objects(container)
        return render_template("list_obj.html", \
                container=container, \
                username="%s:%s"%(Global.group, Global.user),\
...
def delete_obj():
  if Global.isloggedin():
    ...
    container=request.args.to_dict()["container"]
    obj=request.args.to_dict()["obj"]
    app.logger.warning("keys:%s %s----------------"%(container, obj))
    ...
    return render_template("list_obj.html",\
            username="%s:%s"%(Global.group, Global.user),\
            container=container, result=result, details=details) 
  return render_template('login.html')
  ...
  if Global.isloggedin():
    ...
    if not args: #container is None:
      ...
      return render_template("list.html",  
        username="%s:%s"%(Global.group, Global.user), result=result)
    else:
      container=args["container"]
      result=Global.swiftconn.list_objects(container)
      return render_template("list_obj.html",\
        username="%s:%s"%(Global.group, Global.user),\
        container=container, result=result) 
  return render_template('login.html')
...
def login():
  ...
  if request.method == 'POST':
    ...
    if conn.connect():
      Global.setsession(conn, group, user) 
      role=Global.swiftconn.get_role()
      flash('You are logged in as %s:%s  %s'%(group, user, [role[1] if role[0] == 0 else " "][0]), 'success')
...
def logout():
    Global.unsetsession()
    flash('You are logged out', 'block')
