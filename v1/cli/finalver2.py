...
class Connection:
  ...
  def list_containers(self):
    try:
      for item in self.conn.get_account()[1]:
        print "name: %s, obj count: %d, total bytes: %d"%(\
  ...
  def _upload_segment_object(self, container, name, segsize):
    size=os.path.getsize(name)
    if size <=segsize:
      print "file size smaller than segment size: %d <= %d"%(size, segsize)
      ...
      try:
        ...
        try:
          ...
          print 'Error trying to create container %r: %s'%(segcontainer, msg)
          return
        ...
        with open(name, 'rb') as fp:
          while segment_start < fullsize:
            ...
            print "upload segment: %s/%s"%(segcontainer, path) 
            segment += 1
  ...
  def upload_object(self, container,fname, segsize=None):
    ...
    if 0 == ct[0]:
      print "object %s already exists in container %s:"%(fname, container)
      print ct[1]
    else:
      if segsize is None: 
